...
// This class makes the frame and calls another class Constellio1Panel which handles the 
// job done in the game. The instance of Constellio1Panel is also created and added to the frame. 
public class Constellio1
{
	// creates an instance of the class itself and calls the method makingFrame. 
	public static void main(String args[])
	{
		Constellio1 c1 = new Constellio1();
		c1.makingFrame();
		
	}
	
	// This method is responsible to create the frame labeled "Constellio1"
	// It sets the size and location of the frame. 
	// This method also creates an instance of the Constellio1Panel class whose 
	// contents are added to the frame.
	public void makingFrame()
	{
		JFrame frame = new JFrame("Constellio 1"); // makes the frame named Constellio1
		frame.setDefaultCloseOperation(frame.EXIT_ON_CLOSE);
		frame.setSize(1000,1000);
		frame.setLocation(10,100);
		
		Constellio1Panel c1p = new Constellio1Panel();
		frame.getContentPane().add(c1p);
...
class Constellio1Panel extends JPanel
{
...
class HomePanel extends JPanel 
{
	...
	private Constellio1Panel cp;
		
		// The instance of Constellio1Panel and cardLayout is passed so that the listeners 
		// which are nested inside this class are able to access them. 
		public HomePanel(CardLayout cards,Constellio1Panel c1p)
		{
			cp = c1p;
			card = cards;
...
class HowToPlayPanel extends JPanel 
{
	...
	private CardLayout card;
		...
		// The instance of Constellio1Panel and cardLayout is passed so that the listeners 
		// which are nested inside this class are able to access them. 
		public HowToPlayPanel(CardLayout cards,Constellio1Panel c1p)
		{
				cp = c1p;
				card = cards;
				// Text in the North.
...
class ConstellationGuidePanel extends JPanel 
{
		...
		// The instance of Constellio1Panel and cardLayout is passed so that the listeners 
		// which are nested inside this class are able to access them. 
			public ConstellationGuidePanel(CardLayout cards,Constellio1Panel c1p)
			{
...
class LevelsPanel extends JPanel 
{
	...
	// The instance of Constellio1Panel and cardLayout is passed so that the listeners 
	// which are nested inside this class are able to access them.
	public LevelsPanel(CardLayout cards,Constellio1Panel c1p)
	{
			cp = c1p;
			card = cards;
...
class HighScoresPanel extends JPanel 
{
	...
	public HighScoresPanel(CardLayout cards,Constellio1Panel c1p)
	{
...
class GamePanel extends JPanel
{
	...
	public GamePanel(CardLayout cards,Constellio1Panel c1p)
	{
		cp = c1p;
		card = cards;
		countDown = new JLabel("Timer : " + cp.getSeconds());
		height = 300; 
		width = 500;
		...
		JTextArea question = new JTextArea(cp.getQuestions()[questionNumber],3,45);
	...
	{
		super.paintComponent(g);
		countDown.setText("Timer : " + cp.getSeconds());
		imagePanel.repaint();
	...
	class SliderListener implements ChangeListener
	{
		...
		{
			int val = imageSize.getValue()*5;	// get the value of the slider
			imagePanel.updateImage(width + val,height + val,image);
			imagePanel.setPreferredSize(new Dimension(width + val, height + val));
			repaint();
		}
...
class ImagePanel extends JPanel
{
	...
	public ImagePanel(Constellio1Panel c1p,int width,int height,int image)
	{
		cp = c1p;
		widthOfImages = width; 
