...
public class Constellio4
{
	// creates an instance of the class itself and calls the method makingFrame. 
	public static void main(String args[])
	{
		Constellio4 c4 = new Constellio4();  // creates an instance of the Constellio4 class
		c4.makingFrame(); // calls a method making frame which makes the frame.
	...
	{
		JFrame frame = new JFrame("Constellio 4"); // makes the frame named Constellio4
		frame.setDefaultCloseOperation(frame.EXIT_ON_CLOSE);
		frame.setSize(1000,1000);
		frame.setLocation(10,100);
		
		Constellio4Panel c4p = new Constellio4Panel(); // creates an instance of Constellio4Panel class.
		frame.getContentPane().add(c4p); // adds the content of that class to the frame. 
...
class Constellio4Panel extends JPanel
{
	...
	private LevelsPanel levelsCard;
...
class HomePanel extends JPanel 
{
	...
	private Constellio4Panel cp;  // This is the instance of Constellio4Panel.
		...
		public HomePanel(CardLayout cards,Constellio4Panel c4p)
		{
			cp = c4p;
			card = cards;
...
class HowToPlayPanel extends JPanel 
{
	...
	private CardLayout card; // This is the instance of the cardLayout
		...
		// which are nested inside this class are able to access them. 
		public HowToPlayPanel(CardLayout cards,Constellio4Panel c4p)
		{
				cp = c4p;
				card = cards;
				// Text in the North.
...
class ConstellationGuidePanel extends JPanel 
{
	...
	private JPanel[] panels; // This is an array of JPanels that has the CardLayout and is present inside the GridLayout of the centerPanel.
		...
		// The instance of Constellio4Panel and cardLayout is passed so that the listeners 
		// which are nested inside this class are able to access them. 
			public ConstellationGuidePanel(CardLayout cards,Constellio4Panel c4p)
			{
...
class LevelsPanel extends JPanel 
{
	...
	// which are nested inside this class are able to access them.
	public LevelsPanel(CardLayout cards,Constellio4Panel c4p)
	{
			cp = c4p;
			card = cards;
...
{
	private Constellio4Panel cp; 
	private CardLayout card;
	
	// The instance of Constellio4Panel and cardLayout is passed so that the listeners 
	// which are nested inside this class are able to access them.
	public HighScoresPanel(CardLayout cards,Constellio4Panel c4p)
	{
		...
		JTextArea highScoresList = new JTextArea(highScoreText); // writes in the text inside the textArea after reading the whole file.
...
{
	private Constellio4Panel cp; // the instance of the Constellio4Panel class.
	...
	// The instance of Constellio4Panel and cardLayout is passed so that the listeners 
	// which are nested inside this class are able to access them.
	public GamePanel(CardLayout cards,Constellio4Panel c4p)
	{
		cp = c4p;
		card = cards;
		...
		// Label for keeping the track of the score.
	...
	public void paintComponent(Graphics g)
	{
		...
		public void avoidRepeatingQuestions()
		{
			int tempNum = 0; 
			int tempArray[] =  cp.getQuestions().length - 1; 
			
			for(int i = 0; i < cp.getQuestions().length; i++)
			{
				questionNumber = (int)(Math.random()*cp.getQuestions().length); // uses Math.Random to randomly choose a question.
				questionNumber = tempNum; 
				if(questionNumber == i)
				{
					tempArray[tempNum] = 0;
					tempNum++;
			...
			while(tempArray[tempNum] != 0)
			{
				questions[questionNumber] = cp.getQuestions();	
				questionNumber++;
			}
				...
				else if(command.equals("SUBMIT"))
				{
					...
					else 
					{
						...
						else 
						{
							...
							levelScore = 0;
							cp.updateLevelsCard();
							card.show(cp,"Levels"); // changes to the levels panel when all the lives have been lost.
					...
					questionNumber = (int)(Math.random()*cp.getQuestions().length);
...
{
	private Constellio4Panel cp; // instance of the Constellio4Panel.
	...
	public ImagePanel(Constellio4Panel c4p,int width,int height,int image)
	{
		cp = c4p;
		widthOfImages = width; 
...
{
	private Constellio4Panel cp;   // instance of Constellio4Panel
	private int imageNumber; //index of the image that is being displayed.
	
	
	public InfoImage(Constellio4Panel c4p,int image)
	{
		setBackground(Color.DARK_GRAY);
		cp = c4p;
		imageNumber = image;
...
class GuideImage extends JPanel
{
	...
	public GuideImage(Constellio4Panel c4p)
	{
		cp = c4p;
	}
...
class CorrectFrame extends JFrame
{
	...
	public CorrectFrame(Constellio4Panel c4p)
	{
		cp = c4p;
...
class IncorrectFrame extends JFrame
{
	...
	public IncorrectFrame(Constellio4Panel c4p,int qNum)
	{
		...
		label.setFont(new Font("Serif", Font.BOLD, 20));
		String text = "The correct answer is " + c4p.getAnswers()[qNum] + " because " + c4p.getExplanations()[qNum] ;
...
class TaurusImagePanel extends JPanel
{
	...
	public TaurusImagePanel(Constellio4Panel c4p)
	{
		setBackground(Color.DARK_GRAY);
		cp = c4p;	
	}
...
class HighScoreNameFrame extends JFrame
{
	...
	public HighScoreNameFrame(Constellio4Panel c4p,int qNum)
	{
		...
		label.setFont(new Font("Serif", Font.BOLD, 20));
		JTextArea jta = new JTextArea("",3,2);
		jta.setBackground(Color.RED);
		jta.setPreferredSize(new Dimension(500,300));
		jta.setLineWrap(true); 
		jta.setWrapStyleWord(true); 
	...
	class ButtonHandler implements ActionListener
	{
		...
		public HighScoreNameFrame(HighScoreNameFrame frame)
		{
			hsnf = frame;
		}
		public void actionPerformed(ActionEvent evt)
